...
#bootstrap to access the model varibility

boot.fn <- function(data, index = sample(3549,3549,replace = T)){
	return(coef(lm(log_losdays2 ~ is30dayreadmit + cindex + 
                    evisit + ageyear + gender + maritalstatus + 
                    insurancetype + bpsystolic + heartrate +
                    respirationrate + bpdiastolic + o2sat, subset = index, 
	               data = gh_project)))
}

boot.adj.r <- function(data, index = sample(3549,3549,replace = T)){
	return(summary(lm(log_losdays2 ~ is30dayreadmit + cindex + 
                    evisit + ageyear + gender + maritalstatus + 
                    insurancetype + bpsystolic + heartrate +
                    respirationrate + bpdiastolic + o2sat, subset = index, 
	               data = gh_project))$adj.r.squared)
}
...
boot_coef <- boot(gh_project, boot.fn, 100)
boot_adj_r <- boot(gh_project, boot.adj.r, 100)
...
##using LOOCV to access the model prediction
glm.fit <- glm(log_losdays2 ~ is30dayreadmit + cindex + 
                    evisit + ageyear + gender + maritalstatus + 
                    insurancetype + bpsystolic + heartrate +
                    respirationrate + bpdiastolic + o2sat,
	               data = na.omit(gh_project))

cv.err <- cv.glm(na.omit(gh_project),glm.fit)
